...
def y_f(x, x0):
    h = lambda y: x[0] + 10**(y - 14) - 10**(-y) + x[1]*(1 + 2*10**(y - pk2))/(1 + 10**(pk1 - y) + 10**(y - pk2))
    y = fsolve(h, x0=x0)[0]
    return y
...
def main():
    ...
    for n in tqdm(range(0, k)):
        ...
        Y_PID[n+1] = y_f(X_PID[n+1], x0=Y_PID[n]) + np.random.normal(scale=noise_scale)
    ...
    for n in tqdm(range(0, k)):
        ...
        Y_PID2[n+1] = y_f(X_PID2[n+1], x0=Y_PID2[n]) + np.random.normal(scale=noise_scale)
    ...
    for ki in range(t_sim.shape[0]):
        if ki % int(150/dt) == 0:
            U_PID2[:, 1] = np.random.uniform(low=0.0, high=5.0)
